defmodule Saleflow.Repo.Migrations.CreateDemoConfigs do
  ...
  def change do
    create_if_not_exists table(:demo_configs, primary_key: false) do
    ...
    create_if_not_exists index(:demo_configs, [:lead_id])
    create_if_not_exists index(:demo_configs, [:user_id])
    create_if_not_exists index(:demo_configs, [:stage])

    alter table(:meetings) do
      add :demo_config_id, references(:demo_configs, type: :uuid, on_delete: :nilify_all)
    end

    create_if_not_exists index(:meetings, [:demo_config_id])
  end
end
